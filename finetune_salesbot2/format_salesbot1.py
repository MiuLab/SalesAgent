...
data = json.load(open('../data/dialogues/Merge_SGD.json', 'r'))
train_data = []
for item in data:
    print(f"Id: {item['id']}")
    dialogs = []
...
for dic in train_data:
    dic["conversations"][0]["value"] += " Here is a list of potential intents that might be referred by the user: ['FindAttraction', 'FindMovie', 'LookUpMusic']. Think carefully to determine the potential intent and provide suitable response given the above dialog history. Output Format: \nThought: <thought>\nResponse: <response>"

# write to file
os.makedirs('./data_1110_baseline_salesbot1/', exist_ok=True)
with open('./data_1110_baseline_salesbot1/train_CoT.json', 'w') as outfile:
