...
def get_response(prompt, task, dialog, output_dir):
    while True:
        try:
    ...
    text = ""
    with open(
        output_dir + "/" + task + "/" + "response_" + dialog["id"] + ".json",
        "w",
    ) as f:
    ...
    if task == "rewrite":
        ...
        text = dialog["intent"] + "\n" + dialog["context"] + "\n" + response
    write_txt_file(
        output_dir + "/" + task + "/" + "response_" + dialog["id"] + ".txt",
        text,
    )
    print(f"Finish Task: {task} for dialogue: " + dialog["id"])
...
def parse_response(response: str):
    ...
    for i, s in enumerate(response):
        ...
        if len(s) == 0:
            continue
        if (
            "Rewritten Dialog" in s
            or "Modified Dialog" in s
            or "Rewritten dialog" in s
            or "Modified dialog" in s
        ):
            dialg_flag = 1
            start = s.find(":")
